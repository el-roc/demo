// import newAddress from("./student-2.json")
// var newAddress = require("././student-2.json")
...
L.tileLayer('https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token=pk.eyJ1Ijoicm9jLXN0b3JtIiwiYSI6ImNrOTBrcGsyNzAydHgzbnJpN2x5c2lsaTkifQ.tbliwu04DtIgSuTZrPeDFA', {
    ...
    fetch('mark', {
          ...
          .then(data => {
              console.log("after fetch of /mark: ", data)
           data.forEach(mark  => {
               console.log("placing this on a map: ", mark)
            L.marker([mark.coordinates.lat, mark.coordinates.lng]).addTo(mymap)
            .bindPopup(`<a href='/individualpark?id=${mark._id}'><b>${mark.name}</b><br /></a>`).openPopup()
